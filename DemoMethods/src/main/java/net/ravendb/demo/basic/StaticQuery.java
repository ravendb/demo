...
public class StaticQuery {
    ...
    public List<Order> staticQuery(@RequestParam(value = "highPrice", defaultValue = "500") int highPrice,
        ...
        try (IDocumentSession session = DocumentStoreHolder.getStore().openSession()) {
            ...
            List<Order> problematicOrder = session.query(CostlyOrders.Result.class, CostlyOrders.class)
                    .where(c.price.gt(highPrice).and(c.delay.gt(String.format("00:%02d:00", delayDays))))
                    .as(Order.class)
                    .toList();
